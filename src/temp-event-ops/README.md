...
- **Show Open Slots** button (posted on the roster help message, `buildSignupButtonsRow` in `message-formatter.ts`) — ephemeral reply with a select menu of that channel's open slots (up to 25; more can be found via `/event-signup slot:` search). Picking one claims it through the same `performSignup` used by the slash command.
